...
!======================================================================================================!
  ...
  real(dp) :: Ea(1),Edes(atomType),Edif(atomType),ad_rate(2),des_rate(atomType),promoter_energy
  real(dp) :: diffusion_val(atomType),p_atm(2),promote_dif_rate(2,2),promote_rxn_rate(2,2),area,timer(6)
...
!======================================================================================================!
    ...
    Ea = [0.60d0]                      ! Grabow, Mavrikakis : Mechanims of Methanol Synthesis
    Edes = [1.00d0,1.00d0,0.08d0,10.00d0]     ! Grabow, Mavrikakis : Mechanims of Methanol Synthesis
    Edif = [0.60d0,0.60d0,1.00d0,0.750d0]    ! Gomer_90 

    lateral_energy = [-0.10d0,-0.10d0,0.0d0,0.0d0]
    promoter_energy = -0.40d0
...
!======================================================================================================!
  SUBROUTINE reaction_results_sohr(n,total_rate,move,sums_all)
!======================================================================================================!
    implicit none
    integer, intent(inout)  :: n,move
    integer, intent(out)    :: sums_all(2)
    real(dp), intent(inout) :: total_rate
    integer                 :: i,j,x,y,choice,choice_x,choice_y,a,b,c,d,sums,sums_pro

    choice = 0 ; sums = -1 ; sums_pro = -1
    ...
    do i = 1,sym
      ...
      if (move == 4+i) then    ! Reaction A*+B*-> C*
        ...
        if (n == atom_follow) atom_finish = 1 
        if (choice == atom_follow) atom_finish = 1 

        ! want to determine the neighboring environment of follow atom that reacted
        if (atom_finish == 1) then 
          sums = 0 ; sums_pro = 0

          ! searches for A & B for any D neighbors 
          ! if either of the two atoms is B then searches for other B neighbors for environment
          do j = 1,sym
            if (atom(choice)%neighbor(j) > 0) then ! check for D neighbors 

              if (atom(atom(choice)%neighbor(j))%id == 4) sums_pro = sums_pro + 1

              if (atom(choice)%id == 2) then ! if B check for B neighbors
                if (atom(atom(choice)%neighbor(j))%id == 2) sums = sums + 1
              end if             
            end if

            if (atom(n)%neighbor(j) > 0) then ! check for D neighbors

              if (atom(atom(n)%neighbor(j))%id == 4) sums_pro = sums_pro + 1

              if (atom(n)%id == 2) then ! if B check for B neighbors 
                if (atom(atom(n)%neighbor(j))%id == 2) sums = sums + 1
              end if  
            end if 
          end do 
          print*,'Sums',sums,sums_pro
        end if  
        ...
        if (choice == atom_follow) then  
          atom_finish = 2 ; print*,'finish = 2'
        end if 
...
!======================================================================================================!
    implicit none 
    integer, intent(in) :: n
    integer :: i,j,k,e_add(atomType),promote_add,types 

    e_add = 0 ; promote_add = 0 ; types = atom(n)%id
    ...
    do j = 1,sym  ! loop over neighbors to find lateral interactions
      if (atom(n)%neighbor(j) /= 0) then
        ...
        if (atom(atom(n)%neighbor(j))%id == 4 .and. atom(n)%id == 1) promote_add = promote_add + 1
        if (atom(atom(n)%neighbor(j))%id == 4 .and. atom(n)%id == 2) promote_add = promote_add + 1
      end if 
    end do
    ...
    do k = 1,2
      if (promote_add > 0) then 
        do j = 1,sym 
          if (atom(n)%rates(1,j) /= 0.0d0) then
            do i = 1,2
              if (promote_add == i) atom(n)%rates(1,j) = promote_dif_rate(i,k)
            end do 
          end if
          if (atom(n)%rates(2,j) /= 0.0d0) then 
            do i = 1,2
              if (promote_add == i) atom(n)%rates(2,j) = promote_rxn_rate(i,k)
            end do 
          end if 
        end do 
      end if 
...
!======================================================================================================!
    ...
    !======================== Rates with promoter interactions =========================!
    do i = 1,2
      do j = 1,2
      !promote_dif_rate(j,i) = exp(log(10.0D0**13)-(Edif(i)-dble(j)*promoter_energy)/(kBev*T))
      promote_dif_rate(j,i) = exp(log(10.0D0**13)-(Edif(i))/(kBev*T))
      promote_rxn_rate(j,i) = exp(log(10.0d0**13)-(Ea(1)+dble(j)*promoter_energy)/(kBeV*T))
      end do 
    end do 
    !===================================================================================!
