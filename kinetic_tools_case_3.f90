...
!======================================================================================================!
  implicit none
  integer, intent(in) :: atom_select,simType,input(:),limiter,sim
  integer :: i,j,k,t,n,z,move,values(8),surface_dim(2),ntime,counts,ad_val,sizer,atom_choice
  integer, allocatable :: seed(:),species_B(:,:),species_A(:,:),species(:,:,:),id_val(:)
  real(dp), intent(in) :: times(3)
  real(dp) :: randy,val,val2,time,delta_time,total_rate,part_rate,sums,timeStop,finish,start,check_rate
  real(dp) :: pick_time,follow_time
  real(dp), allocatable :: surface_species_time(:),spline_saver(:,:)
  ...
  if ((promoter_energy == 0)) then 
    if (.not. allocated(id_val)) allocate(id_val(1))
  else 
    if (.not. allocated(id_val)) allocate(id_val(2))
  end if 
